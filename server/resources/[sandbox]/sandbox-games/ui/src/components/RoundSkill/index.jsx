...
import keymap from '../../containers/Minigames/keymap';
...
export default ({ game }) => {
    ...
    useKeypress(keymap.filter((k) => !game.key.includes(k)), () => {
        if (pressed) {
            return;
        }
    ...
    useKeypress(keymap.filter((k) => !game.key.includes(k.toUpperCase())), () => {
        if (pressed) {
            return;
        }
