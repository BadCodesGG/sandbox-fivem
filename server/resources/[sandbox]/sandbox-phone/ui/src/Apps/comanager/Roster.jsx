					...
					if (res.success) {
						...
						switch (res.code) {
							...
							case 'OUTSTANDING_OFFER':
								sendAlert(
									'Person Has An Outstanding Employment Offer',
								);
								break;
						}
					}
		...
		try {
			...
			if (res.success) {
				...
				switch (res.code) {
					...
					case 'ERROR':
						sendAlert(
							`Unable To Fire ${hiring.First} ${hiring.Last}`,
						);
						break;
				}
			}
		...
		>
			...
			{hireEmployee && Boolean(hiring) && (
				<Modal
					form
					open={hiring != null}
					title={hiring?.edit ? `Update Employment` : `Hire Employee`}
					onClose={() => setHiring(null)}
					onAccept={onHire}
					onDelete={
						hiring?.edit &&
						(manageCompany || fireEmployee) &&
						hiring?.SID != player.SID
							? () => setFiring(true)
							: null
					}
					deleteLang="Fire"
			...
			<Modal
				...
				onDelete={!isOwner ? () => setQuitConf(true) : null}
				deleteLang="Quit"
			>
				<List>
					<ListItem>
