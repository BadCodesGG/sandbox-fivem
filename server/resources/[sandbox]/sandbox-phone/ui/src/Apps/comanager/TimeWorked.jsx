...
import { List, AppBar, Grid, IconButton, Tooltip } from '@mui/material';
...
const parseLastDutyTime = (lastDuty, job) => {
	if (lastDuty && lastDuty[job]) {
		return `${moment(lastDuty[job] * 1000).format('LLL')} (${moment(
			lastDuty[job] * 1000,
		).fromNow()})`;
	} else {
		return 'Never';
...
export default ({ jobData, playerJob }) => {
	...
	const timeWorkedUpdated = useSelector(
		(state) => state.com.timeWorkedUpdated,
	);
	...
	useEffect(() => {
		if (
			!timeWorked ||
			Date.now() - timeWorkedUpdated >= 120000 ||
			timeWorkedJob != jobData.Id
		) {
			fetchTimeWorked();
		}
	...
	return (
		<AppContainer
			appId="comanager"
			titleOverride={jobData.Workplace ? jobData.Workplace?.Name : jobData.Name}
			actions={
				<Fragment>
					<Tooltip title="Home">
		...
		>
			...
			>
				{viewing != null && (
					<>
						<p>
							{viewing.First} {viewing.Last} ({viewing.SID})
						</p>
						<p>
							Last Went on Duty:{' '}
							{parseLastDutyTime(viewing.LastClockOn, jobData.Id)}
						</p>
						<p>
							Has worked{' '}
							{parseDutyTime(viewing.TimeClockedOn, jobData.Id)}{' '}
							in the last week.
						</p>
					</>
				)}
