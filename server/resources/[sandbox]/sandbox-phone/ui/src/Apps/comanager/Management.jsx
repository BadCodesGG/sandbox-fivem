...
import { Modal, Confirm, Loader, AppContainer } from '../../components';
...
export default ({ refreshRoster, jobData, playerJob }) => {
	...
	const jobPermissions = useSelector(
		(state) => state.data.data.NamedJobPermissions,
	);
	...
	return (
		<AppContainer
			appId="comanager"
			titleOverride={
				jobData.Workplace ? jobData.Workplace?.Name : jobData.Name
			}
			actions={
				<Fragment>
					{isOwner && (
		...
		>
			...
			>
				<p>
					Deleting this grade will remove all permissions related to
					it and cannot be undone, are you sure?
				</p>
			</Confirm>
			<Modal
				form
						...
						<FormControl fullWidth className={classes.editorField}>
							...
							<Select
								...
								renderValue={(selected) =>
									selected
										.map(
											(k) => jobPermissions[k]?.name ?? k,
										)
										.join(', ')
								}
							>
								{Object.keys(jobPermissions)
									.filter((p) => {
										const pData = jobPermissions[p];
										return (
											!pData.restricted ||
											pData.restricted?.includes(
												jobData?.Id,
											)
										);
									})
			...
			{isOwner && (
				<>
						...
						<>
							...
							>
								<p>
									...
									ownership of company & associated assets.
								</p>
								<p>
									This is not reversable, if you proceed you
									will lose all control over the company.
								</p>
							</Confirm>
						</>
					)}
					...
					>
						...
						<p>
							This is not reversable, if you proceed this company
							will be lost forever and you{' '}
							<b>
								<i>WILL NOT</i>
							</b>{' '}
							get any sort of refund for anything purchased for
							your company.
						</p>
					</Confirm>
				</>
