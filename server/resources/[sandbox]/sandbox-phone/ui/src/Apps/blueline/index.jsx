...
export default (props) => {
	...
	const tracks = useSelector((state) => state.data.data.tracks_pd) ?? Array();
	const rawRaces = useSelector((state) => state.pdRace.races);
	const races = Object.keys(rawRaces)
		.reduce((result, key) => {
			if (
				Boolean(rawRaces[key]) &&
				rawRaces[key]?.state != -1 &&
				rawRaces[key]?.state != 2
			)
	...
	return (
		<AppContainer
			appId="blueline"
			titleOverride="Trials - Pending Races"
			actionShow={Boolean(alias) && onDuty == 'police'}
			actions={
				<Fragment>
					<IconButton
						onClick={() =>
							navigate('/apps/blueline/completed', {
								replace: true,
							})
						}
					>
