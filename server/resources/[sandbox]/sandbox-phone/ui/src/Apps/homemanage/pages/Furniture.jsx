...
export default ({ property, onRefresh, myKey }) => {
	...
	useEffect(() => {
		...
		const f = async () => {
			try {
				let res = await (await Nui.send('Home:GetCurrentFurniture', { property })).json();
				if (res && res.success) {
	...
	const placeFurniture = async (category, model) => {
		try {
			let res = await (
				await Nui.send('Home:PlaceFurniture', { category, model })
			).json();
	...
	const findFurniture = async (id) => {
		try {
			let res = await (
				await Nui.send('Home:HighlightFurniture', { id })
			).json();
	...
	const editFurniture = async (id) => {
		try {
			let res = await (
				await Nui.send('Home:EditFurniture', { id })
			).json();
	...
	const deleteFurniture = async (id) => {
		try {
			let res = await (
				await Nui.send('Home:DeleteFurniture', { id })
			).json();
	...
	if (!myKey?.Permissions?.furniture && !myKey?.Owner) {
		return (
			<div className={classes.wrapper}>
				<div className={classes.emptyMsg}>Invalid Permissions</div>
			</div>
		);
	} else {
		return (
			<div className={classes.wrapper}>
				{err != null || !furniture ? (
					<div className={classes.emptyMsg}>{err}</div>
				) : (
				...
				{Boolean(choosing) && (
					...
					>
						<p>
							<i>
								You can browse through a category once in
								placement mode by using your arrow keys!
							</i>
						</p>
						...
						/>}
						{selectedCat === "search" ? (
							<List className={classes.items2}>
								{Object.keys(catalog)
									.filter((m) => catalog[m].name.toLowerCase().includes(search.toLowerCase()) || catalog[m].model.toLowerCase().includes(search.toLowerCase()))
							...
							<List className={classes.items}>
								{Object.keys(catalog)
									.filter((m) => catalog[m].cat == selectedCat)
									.sort((a, b) => catalog[a].id - catalog[b].id)
									.map((m) => {
										return (
											<ListItem
				...
				{err == null && Boolean(furniture) && <Fab className={classes.fab} onClick={() => setChoosing(true)}>
