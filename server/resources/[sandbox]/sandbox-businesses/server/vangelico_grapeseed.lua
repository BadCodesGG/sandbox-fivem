local _jobName = "vangelico_grapeseed"

AddEventHandler("Businesses:Server:Startup", function()
	Callbacks:RegisterServerCallback("Businesses:VANGELICOGRAPESEED:OpenTable", function(source, data, cb)
		Inventory:OpenSecondary(source, 221, data)
	end)

	Callbacks:RegisterServerCallback("Businesses:VANGELICOGRAPESEED:Sell", function(source, data, cb)
...
AddEventHandler("Businesses:Server:VANGELICOGRAPESEED:ViewGem", function(source, data)
	local char = Fetch:CharacterSource(source)
	if char ~= nil then
		if Jobs.Permissions:HasJob(source, _jobName, false, false, false, true, "JOB_USE_GEM_TABLE") then
			...
			if #its > 0 then
				...
				if itemData ~= nil and itemData.type == 11 and itemData.gemProperties ~= nil then
					TriggerClientEvent(
						"Businesses:Client:VANGELICOGRAPESEED:ViewGem",
						source,
						data.owner,
