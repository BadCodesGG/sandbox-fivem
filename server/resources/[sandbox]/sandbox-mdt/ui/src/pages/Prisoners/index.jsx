...
export default () => {
    ...
    const [reducing, setReducing] = useState(null);
    ...
    const onStartReduction = (prisoner) => {
        setReducing(prisoner);
    };

    const onReduction = async (data) => {
        setLoading(true);
        setReducing(null);

        try {
            let res = await (
                await Nui.send('DOCReduceSentence', data)
            ).json();
            if (res) {
            ...
            console.log(err);
            toast.error('Error Reducing Sentence');
        }
    ...
    return (
        <div className={classes.wrapper}>
                                                ...
                                                .map((result) => {
                                                    return (
                                                        <Item
                                                            key={result.id}
                                                            prisoner={result}
                                                            onStartReduction={onStartReduction}
                                                        />
                                                    );
                                                })}
                                        </List>
            ...
            <ReductionForm
                open={Boolean(reducing)}
                data={reducing}
                onClose={() => setReducing(null)}
                onSubmit={onReduction}
            />
        </div>
    );
};
